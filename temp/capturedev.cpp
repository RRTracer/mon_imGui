...
#ifdef NATIVE_BUILD
     // Linux
     // + FIXME : test other possibilities, e.g. using GSTREAMER (cv::CAP_GSTREAMER)
...
//    maVideoCap.open(maDevNumber + cv::CAP_MSMF);
#endif
    ...
    const char * thermal_camera = getenv("THERMAL_CAMERA");

    // experimental, testing Optris 640 pi thermal camera
    if (!std::strcmp(thermal_camera, "OPTRIS_PI"))
    {
        // TODO later
        std::cerr << "Optris thermal camera detected." << "\n";
    }
...
#else
    setCaptureFPS(WINDOWS_DEFAULT_FRAMERATE);
#endif
...
void CaptureDev::setFourCC(unsigned int FourCC)
{
    ...
    maVideoCap.set(cv::CAP_PROP_FOURCC, FourCC);
}

// TODO later
void CaptureDev::setConvertRGB(bool bConvertRGB)
{
    maVideoCap.set(cv::CAP_PROP_CONVERT_RGB, bConvertRGB);
}
